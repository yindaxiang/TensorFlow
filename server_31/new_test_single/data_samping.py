...
#                                          y_: y_train[start: start + BATCH_SIZE], keep_prob: 0.5})
            return [x_train[start: start + BATCH_SIZE], y_train[start: start + BATCH_SIZE], 0.5]
...
def get_train_batch_tesra(step=0):
    graph = 0
    data_list_tesra = train_model(graph, step)
    batch_x = np.array(list(data_list_tesra[0]))
    batch_y = np.array(list(data_list_tesra[1]))
    return batch_x, batch_y


keep_prob_value = 0.500000
